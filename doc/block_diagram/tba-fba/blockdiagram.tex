...
\begin{tikzpicture}[node distance=2.5cm,auto,>=latex']
    ...
    \node [int] (smoothing) [align=left, right of=e] {smoothing \\low pass filters};
    \node [int] (f) [align=left, right of=smoothing] {trend \\subtraction};
    \node [int] (g) [align=left, right of=f] {half wave \\rectification};
    \node [int, pin={[init]above:\hspace{1.5cm}lo \hspace{0.25em}|\hspace{0.25em} hi |\hspace{0.5em} GUI}] (h) [align=left, right of=g] {threshold \\detection};
    ...
    \node [int, pin={[init]below:\hspace{3.75cm}FBA threshold | \\GUI}] (o) [align=left, below of=d, node distance=8cm] {ODFT};
    \node [int] (p) [align=left, right of=l, node distance=10cm] {sample player*};
    ...
    \draw[->] (c) edge node {} (d);
    \draw[->] (d) edge node {} (e);
    \draw[->] (e) edge node {} (smoothing);

    %\draw[->] (e) edge node {} (f);
    %\draw[->] (f) edge node {} (g);
    \draw[->] (smoothing) edge node {} (f);
    \draw[->] (f) edge node {} (g);
    \draw[->] (g) edge node {} (h);
    \draw[->] (h) edge node {} (i);
    \draw[->] (j) edge node {} (i);
